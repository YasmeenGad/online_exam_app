...
class QuestionsOfflineDatasourceImpl implements QuestionsOfflineDatasource {
  ...
  Future<void> cacheQuestions(
      QuestionResponseEntity questions, String examId) async {
  ...
  @override
  Future<void> cacheCheckQuestions(
      CheckQuestionResponseEntity checkQuestion) async {
  ...
  Future<CheckQuestionResponseEntity?> getCachedCheckQuestions(
      CheckQuestionRequestEntity checkQuestion) async {
    final box = await Hive.openBox(_checkQuestionBox);
    final data = box.get(_checkQuestionKey);

    if (data != null) {
      return CheckQuestionResponseEntity.fromJson(
